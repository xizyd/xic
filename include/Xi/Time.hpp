...
// Global Functions provided by Spatial.hpp
i64 epochMicros();
int getGMT();
...
private:
  ...
  // Forward declarations of Global Spatial Functions
  friend i64 epochMicros();
  friend int getGMT();

public:
  int tz = 0; // Timezone Offset in Hours

  /**
   * Suspends the current execution context for a specified duration.
   * This function is non-blocking for the system scheduler.
   */
  static void sleep(double seconds) {
// 1. FreeRTOS Priority (for ESP32 or Embedded Linux with RTOS patches)
...
// 3. Desktop OS (Fedora / Windows)
#else
#if defined(_WIN32)
    ::Sleep(static_cast<DWORD>(seconds * 1000.0));
#else
    // High-precision nanosleep for your Fedora Workstation
    struct timespec ts;
  ...
  static void syncClock(i64 now) {
    i64 uptimeMicros = Xi::micros();
    Xi::systemStartMicros = now - uptimeMicros;
  }
  ...
  // epochMicros() removed: Use global ::Xi::epochMicros() instead.

public:
  // -------------------------------------------------------------------------
  // The Source of Truth
  // -------------------------------------------------------------------------
  i64 us; // Microseconds since Epoch (1970-01-01 00:00:00 UTC)
  ...
  // Default: Now
  Time() {
    us = ::Xi::epochMicros(); // Uses global wrapper (from Spatial.hpp/cpp)
    tz = ::Xi::getGMT();      // Uses global wrapper from Spatial
  }
  ...
  Time(const Xi::String &date, const Xi::String &fmt) : us(0) {
    ...
    // Calculate Epoch
    long long totalDays = daysFromCiv(Y, M, D);
  ...
  } // Total seconds (overflows int likely)
  ...
  void updateDate(int y, int m, int d) {
    long long days = daysFromCiv(y, m, d);
...
public:
  ...
  Property<Time, &Time::getMinuteInHour, &Time::setMinuteInHour> minuteInHour{
      this};
  Property<Time, &Time::getSecondInMinute, &Time::setSecondInMinute> second{
      this};
  Property<Time, &Time::getSecondInMinute, &Time::setSecondInMinute>
      secondInMinute{this};
