...
#endif
#else
#warning "Could not detect if GLFW is available to use... try defining GLFW_AVAILABLE yourself..."
#define GLFW_AVAILABLE 0
#endif
    ...
    public:
        GLFWWindowImpl()
        {
            ...
            glfwSetKeyCallback(_win, [](GLFWwindow *w, int k, int s, int a, int m)
                 ...
                 if(a == GLFW_PRESS) {
                     InputControl c; c.code = k; c.type = InputType::Key; 
                     c.down = true; c.held = true; c.value = 1.0f;
                     me->controls.push(c);
                 } });
        ...
        void update() override
        {
            ...
            for (usz i = 0; i < controls.length; ++i)
            {
                ...
                controls[i].up = (glfwGetKey(_win, controls[i].code) == GLFW_RELEASE && controls[i].held);
            }
