...
1.  **Tunnel:** The Point-to-Point connection manager (Security, Reliability, Heartbeats).
2.  **Railway:** The Mesh Routing and Multiplexing engine (Stations, Anycast, Hubs).
...
- You can pipe a Rho `Tunnel` directly into an OS UDP Linux Socket.
- You can pipe a Rho `Tunnel` into an ESP32's `WiFiUDP` object.
- You can pipe a Rho `Tunnel` into a LoRa `SPI` radio transceiver driver directly over the airwaves.
- You can pipe a Rho `Tunnel` directly across a physical `Serial` or USB UART wire.
...
`Rho::Tunnel` operates on UDP. It allows the developer to explicitly mark individual packets as either **Reliable** (resend if lost) or **Unreliable** (fire and forget). Furthermore, it processes state instantly and independent of order if the packet's `.bypassHOL` is conceptually allowed. It's essentially a deterministic, zero-allocation alternative to **ENet** or **GameNetworkingSockets**.

Additionally, TLS handshakes require multiple round-trips (RTT) to exchange certificates and synthesize keys. `Rho` uses a highly optimized, single-round-trip Ephemeral X25519 Key Exchange (powered by Monocypher), establishing a secure connection significantly faster than TLS 1.3.
...
`Rho` offers QUIC's core benefits (Head-of-Line bypass over UDP, integrated encryption) but does so with absolutely zero dynamic `malloc` calls, making it fast enough for a server and small enough for an embedded RTOS.
